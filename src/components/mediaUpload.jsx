...
export default function MediaUpload({
  ...
  const handleSelectMedia = () => {
    if (selectedExistingMedia?.id) {
      const media = existingMedia.find((item) => item?.id === selectedExistingMedia?.id);
  ...
  return (
    <>
      ...
      />
      {(uploadedMedia?.mimeType?.includes("video") || selectedMedia?.mimeType?.includes("video")) ? (
        <video
        ...
        <img
          src={uploadedMedia?.url || selectedMedia?.url || "/assets/gallery_icon.png"}
          alt="Work Preview"
          className={`object-cover ${uploadedMedia?.url || selectedMedia?.url ? "w-full h-full" : "w-10 h-10"}`}
        />
      )}
      {mediaLoading ? (
        ...
        <div className={`overlay-button absolute inset-0 flex ${path === "profile" ? "flex-col gap-1" : "flex-wrap"} items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity z-10`}>
          <button
            type="button"
          ...
          <div className="relative top-4 max-w-3xl w-full">
            <div className="relative bg-white h-[85vh] py-4 overflow-hidden rounded-md flex flex-col">
              {loading && page === 1 ? (
                ...
                <>
                  <h2 className="text-lg px-8 mb-4 font-semibold">Select Media</h2>
                    ...
                    {existingMedia.map((media, index) => (
                      ...
                      >
                        {media?.mimeType?.includes("video") ? (
                          <>
                            <video className="w-full h-32 sm:h-40 md:h-48 object-cover cursor-pointer rounded-md" autoPlay muted loop>
                          ...
                          <img src={media?.url} alt={media?.filename} className="w-full h-32 sm:h-40 md:h-48 object-cover cursor-pointer rounded-md" />
                        )}
                      </div>
                    ))}
                  </div>
            ...
            <button className="absolute -top-3 -right-3" onClick={() => { setPage(null); setIsMediaModal(false); }}>
              <img src="/assets/close_icon.png" alt="Close icon" className="w-6" />
            </button>
          </div>
        </div>
      )}
